...
**The Flash Cart adds two `wontfix` entries, A75 and A76**, both agreed across
the rollout rather than particular to one emulator: open bus on an unfitted
second chip reads `$FF` in all three, and two of the three complete a flash
...
### A73 — The emulator never modelled CTS, so the far end could never stop it — **fixed**

| | |
|---|---|
| **Claim** | `docs/reference/connectors.md` names `CTS EN` and `DCD EN` and says what moving them does to a real R6551. The emulator the site's samples run on had the same chip with CTS tied to ground by fiat — its own `ACIA.ts`: "Every board ties it low or to the cable's CTS ('CTS EN'), so the transmitter is never disabled by it here" — and DCD and DSR read as constants. So nothing the site said about the handshake jumpers could be tried in the emulator. |
| **Truth** | Measured on a real KIM on 2026-09-18 (Serial Card Pro, real R6551, BIOS 1.6, 19,200 baud, the host driving the lines through a characterised null-modem). CTS high stops the transmitter within about 26 ms. A byte written during the gate is held one deep in the data register with TDRE clear, and a second write overwrites it: four writes, one byte out on release, the last. Software that waits on TDRE therefore blocks and loses nothing. DCD high stops the receiver, and a byte arriving meanwhile is lost. DSR high is a status bit and gates nothing. Status bits 5 and 6 track the pins, active low: `$10` at rest, `$70` with both high, `$50` with `DCD Select` moved to ground. A reset with CTS already high printed nothing for fourteen seconds, then the whole banner the moment CTS returned. |
| **Source** | The bench run, recorded in `6502-EMULATOR/src/core/IO/ACIA.ts` on `transmitterEnabled` and `receiverEnabled`. The pin wiring per card is in `6502-EMULATOR/src/core/IO/SerialCard.ts`, read from the 6502-COB and 6502-ACE schematics. |
| **Check** | RUN — emulator 3.3.0: `6502 run --headless --cts cable` with the console's CTS dropped prints nothing, then BIOS 1.6's whole banner once it returns. With `--cts ground` the same run is not held. The full sample suite, 153 cases, passes unchanged at the defaults. |
| **Status** | `fixed` — `6502-EMULATOR` v3.3.0 and `6502-KIMULATOR` v1.2.0 model the card and its jumpers, and CTS and DCD gate the chip as measured. Both default to ground, where nothing changes. The PicoCalc and the DEV's DB Emulator model a standard Serial Card with its lines at ground, where CTS never gates anything, so neither has one to model. `docs/reference/connectors.md` now says the emulator has the same two jumpers. |
| **Consequence** | Nothing on the site was wrong about the hardware, and no sample could have caught this: every sample runs at the defaults, where a grounded CTS and a missing one behave the same. It matters to a reader who moves a jumper on a real board and wants to see what that costs before building a cable, and to anyone who read the emulator's silence about CTS as the chip's. |

### A74 — The emulator chapter described a port setting that no longer exists, and a terminal the emulator never was — **fixed**

| | |
|---|---|
| **Claim** | `docs/using/emulator.md`, under *Talking to real hardware*: with a port open you "can use it as the terminal for a real ACE", and **Settings → SERIAL** "has two flow-control settings, one for each end of that cable": **Flow Control** for the port and **Emulated machine: RTS/CTS flow control** for the ACE. `docs/using/serial.md` said the same of the emulator as a terminal, "its port is set to RTS/CTS already". |
| **Truth** | A port the emulator opens is wired to the *emulated* machine's serial card. Bytes from the port go into the emulated ACIA and the machine's output goes out of the port, so a real board on the other end is talking to a second ACE, not to a terminal. From 3.3.0 the machine drives the port's RTS itself and reads its CTS, DCD and DSR, and the port opens with the OS's own RTS/CTS off, because it would fight the machine for the line. The port's **Flow Control** setting is gone, `--serial-flow` does nothing, and the other setting is now **Terminal honours RTS**, `--peer-rts honour\|ignore`, with `--[no-]flow-control` kept as a deprecated spelling. |
| **Source** | `6502-EMULATOR` v3.3.0 — `src/renderer/src/composables/useSerial.ts`, `src/main/serial.ts`, `src/renderer/src/components/SettingsPanel.vue`, `src/cli/args.ts`. |
| **Check** | RUN — `6502 run --help` on 3.3.0, and `6502 dbg info` on a machine started with `--peer-rts ignore`, which still reads `no flow control`. |
| **Status** | `fixed` — this repository, in the same commit as the pin. The emulator chapter says what the port is wired to, drops the port setting, and describes the card, its jumpers and **Terminal honours RTS** by the panel's labels. `docs/using/serial.md` no longer offers the emulator as a terminal for a board. The flag tables, `agents.md`, `build-run-loop.md`, `debugging.md`, `samples/README.md` and `samples/crossdev/test.sh` name `--peer-rts ignore`. A71 is left as it was: it is the record of 3.2.0. |
| **Consequence** | The old text sent a reader with a real ACE to the emulator as its terminal, which would have put two machines on one cable, and it described a setting that had been the OS guarding its own buffer for a machine it knew nothing about. Nothing was lost, but a reader debugging a cable would have been changing the wrong thing. |

### A75 — Open bus on a cartridge's unfitted second chip reads `$FF` everywhere — **wontfix**

| | |
|---|---|
| **Claim** | *Bigger cartridges* and the Flash Cart linker configs say a one-chip cart has 15, 31 or 63 selectable banks, and say nothing about what the bank values above that do. A reader who writes one finds out by trying it. |
