...
quietly loses lines. The [emulator](/using/emulator) can't be that terminal:
a serial port it opens is wired to the *emulated* ACE, so a real board on the
other end is talking to a second ACE, not to you.
:::
