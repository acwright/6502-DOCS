...
```
headless 3.5.0 — serial console, 1 MHz, turbo, 1619280 cycles
```
...
with `--peer-rts ignore` — flow control being on is the ordinary case, so the
line says nothing about it. The same goes for the serial card: a machine started
with a jumper moved, `--cts cable` say, gets `ACE (CTS EN: cable, DCD EN:
ground)` in the line, and one on the default card gets nothing. `dbg info
--json` has all three as `vdp`, `flowControl` and `serialCard`, which is the way
for a script to check it started the machine it meant to.
